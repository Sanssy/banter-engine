...
func generate(op opportunities.Opportunity, description string) string {
	...
	case opportunities.CrowdFavorite:
		return fmt.Sprintf("👥 %s concentre %s des pronostics.", op.Actor, op.Target)
	case opportunities.CrowdTrap:
		return fmt.Sprintf("🪤 La foule choisit %s plutôt que le favori %s.", op.Actor, op.Target)
	case opportunities.PopularMistake:
		return fmt.Sprintf("😬 Le choix populaire %s s'effondre sur %s.", op.Actor, op.Target)
	...
	case opportunities.GoalSwing:
		return fmt.Sprintf("⚽ %s fait basculer le score sur %s.", op.Actor, op.Target)
	case opportunities.MatchTurnaround:
	...
	case opportunities.ComebackSeason:
		return fmt.Sprintf("🚀 %s remonte de %s places au classement.", op.Actor, op.Target)
	case opportunities.FreeFall:
